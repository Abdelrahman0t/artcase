...
const LikedPage = () => {
  ...
  const [theSavedDesign, setTheSavedDesign] = useState<any | null>(null);
  ...
  const fetchLikedDesigns = async () => {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/liked/`, {
      method: 'GET',
      headers: {
    ...
    if (response.ok) {
      ...
      setLikedDesigns(data.liked);
    } else {
      console.error('Failed to fetch liked designs');
    }
  ...
  const addToCart = async (designId : any) => {
    ...
    const deleteLikedDesign = async (designId: number) => {
  const token = localStorage.getItem("token");

  const response = await fetch(
    `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/delete-like/${designId}/`,
    {
      method: "DELETE",
      headers: {
  ...
  if (response.ok) {
    alert("Removed from liked designs.");
    setLikedDesigns((prev) => prev.filter((design) => design.id !== designId));
     window.location.reload();

  } else {
    const data = await response.json();
    alert(data.error || "Failed to remove liked design.");
  }
};
  ...
  return (
    <Layout>
      <h2 className={styles2.h2title}>Your Liked Designs</h2>
      {loading ? (
        <p>Loading...</p>
      ) : (
            ...
            <div key={design.id} className={`${styles2.cards}`}>
              ...
              >
                <button
                  ...
                  onClick={() => deleteLikedDesign(design.id)}
                >
                  Remove
                </button>
                <button
